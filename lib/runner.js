...
export default function Jasmine2xRunner ({testPaths, buildAtomEnvironment, buildDefaultApplicationDelegate}) {
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    const paths = globs.map((specPath) => {
      ...
      return path.relative(path.resolve(path.join(__dirname, '../../')), actualPath)
    })
